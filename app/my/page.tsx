...
export default async function MyPage() {
  ...
  return (
    <main className="page-shell py-10 sm:py-14"><div className="mx-auto max-w-6xl">
      <div className="rounded-[2rem] bg-[#1822ff] p-7 text-white sm:p-10"><p className="text-sm font-bold text-white/55">MY STUDIO</p><h1 className="mt-3 text-4xl font-black tracking-[-0.05em] sm:text-6xl">{user?.name ?? "Creator"}의 작업실</h1><div className="mt-8 grid grid-cols-3 gap-3"><div className="stat"><strong>{data.owned.length}</strong><span>올린 영감</span></div><div className="stat"><strong>{data.saved.length}</strong><span>저장한 영감</span></div><div className="stat"><strong>{data.projects.length}</strong><span>내 프로젝트</span></div></div></div>
      <section className="mt-12"><div className="section-title"><div><p className="eyebrow text-[#1822ff]">CREATED</p><h2>내가 올린 영감</h2></div><Button asChild variant="outline" className="rounded-full"><Link href="/inspirations/new"><Plus />새 영감</Link></Button></div>{data.owned.length ? <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.owned.map((item) => <InspirationCard key={item.id} item={item} />)}</div> : <Empty icon={Plus} text="첫 영감을 기록해보세요." href="/inspirations/new" />}</section>
      <section className="mt-14"><div className="section-title"><div><p className="eyebrow text-[#1822ff]">SAVED</p><h2>저장한 영감</h2></div><Bookmark className="text-black/20" /></div>{data.saved.length ? <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.saved.map((item) => <InspirationCard key={item.id} item={item} />)}</div> : <Empty icon={Bookmark} text="마음에 드는 영감을 저장해보세요." href="/" />}</section>
      <section className="mt-14"><div className="section-title"><div><p className="eyebrow text-[#1822ff]">COLLAB</p><h2>내 프로젝트</h2></div><FolderKanban className="text-black/20" /></div>{data.projects.length ? <div className="mt-6 grid gap-3">{data.projects.map((project) => <Link key={project.id} href={`/inspirations/${project.inspirationId}`} className="flex items-center justify-between rounded-2xl border border-black/10 bg-white p-5 hover:border-[#1822ff]"><div><p className="font-black">{project.title}</p><p className="mt-1 text-sm text-black/50">{project.status} · {project.rolesNeeded.join(", ")}</p></div><ArrowRight /></Link>)}</div> : <Empty icon={FolderKanban} text="영감에서 협업 프로젝트를 시작해보세요." href="/" />}</section>
    </div></main>
  );
}
