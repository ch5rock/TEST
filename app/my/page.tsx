...
export default async function MyPage() {
  ...
  return (
    <main className="page-shell py-10 sm:py-14"><div className="mx-auto max-w-6xl">
      <div className="rounded-[2rem] bg-[#1822ff] p-7 text-white sm:p-10"><p className="text-sm font-bold text-white/55">FOOTPRINT</p><h1 className="mt-3 text-4xl font-black tracking-[-0.05em] sm:text-6xl">{user?.name ?? "Creator"}의 작업실</h1><div className="mt-8 grid grid-cols-3 gap-3"><div className="stat"><strong>{data.owned.length}</strong><span>올린 영감</span></div><div className="stat"><strong>{data.saved.length}</strong><span>저장한 영감</span></div><div className="stat"><strong>{data.projects.length}</strong><span>내 프로젝트</span></div></div></div>
