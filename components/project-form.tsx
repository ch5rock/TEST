...
export function ProjectForm({ inspirationId, inspirationTitle }: { inspirationId: string; inspirationTitle: string }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-6">
      <div className="rounded-2xl bg-[#e7e8ff] p-4 text-sm">
        <span className="font-semibold text-[#1822ff]">시작 영감</span><p className="mt-1 font-bold">{inspirationTitle}</p>
      </div>
      ...
      <Button type="submit" size="lg" disabled={loading} className="h-13 rounded-full bg-[#1822ff] text-white hover:bg-[#1119cc]">{loading ? "만드는 중…" : "협업 모집 시작"}<ArrowRight /></Button>
    </form>
  );
}
